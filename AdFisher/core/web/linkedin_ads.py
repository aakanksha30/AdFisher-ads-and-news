...
class LinkedInAdsUnit(browser_unit.BrowserUnit):
  ...
  def search_term(self):
    ...
    try:
      search_box = self.driver.find_element_by_id("body.render-mode-BIGPIPE.nav-v2.ember-application.color-palette.color-palette--control.boot-complete.icons-loaded:nth-child(2) header.extended-nav.nav-main-container.global-alert-offset-top.is-loading-nav:nth-child(2) div.nav-main__content.full-height.display-flex.align-items-center form.nav-search div.nav-search-bar div:nth-child(1) div.nav-search-typeahead artdeco-typeahead-deprecated.search-typeahead-v2.ember-view artdeco-typeahead-deprecated-input.ember-view > input:nth-child(1)")
      search_box.send_keys(self.occupation, keys.ENTER)
  ...
  def save_ads_linkedin(self, file_name=None):
    ...
    lis = self.driver.find_elements_by_css_selector('#ember2840>li a')

    for li in lis:
      try:
        company_title = li.find_element_by_css_selector(' p.subline-level-1-js.mt1.t-14.t-black--light.t-normal.truncate').get_attribute('innerHTML')
        location = li.find_element_by_css_selector('p.subline-level-2-js.t-12.t-black--light.t-normal.truncate').get_attribute('innerHTML')
