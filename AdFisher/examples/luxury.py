...
import web.magic_bricks               # collecting ads

log_file = 'luxury.log.txt'

# Defines the browser that will be used as a "unit" and gives it a copy of the adblock_rules
def make_browser(unit_id, treatment_id):
    b = web.magic_bricks.MagicBricksUnit(log_file=log_file, unit_id=unit_id, 
        treatment_id=treatment_id, headless=False, browser="firefox")
    return b
...
def measurement(unit):
    ...
    sites = ['magicbricks']
    for site in sites:
        unit.collect_ads(site=site, reloads=2, delay=20)
